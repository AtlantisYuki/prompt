...
- Codex：`scripts/install_codex_hook.ps1` → `~/.codex/hooks.json` + `hooks/sdlc_codex_session_start.ps1`
- Claude Code：将 `scripts/hooks/cc_session_start.ps1` 按 Claude settings 的 SessionStart 配置接入
- Grok：`scripts/install_grok_hook.ps1` → `~/.grok/hooks/sdlc-session-start.json` + `hooks/sdlc_grok_session_start.ps1`
